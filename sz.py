...
def gen_stats(base_path="."):
  table = []
  for path, _, files in os.walk(os.path.join(base_path, PACKAGE_DIR)):
    if "__pycache__" in path.replace('\\', '/').split('/'): continue
    for name in files:
      if not (name.endswith(".py") or name.endswith(".js")): continue
      relpath = path.replace('\\', '/')
      if any(s in relpath for s in SKIP_PATHS): continue
      filepath = os.path.join(path, name)
      relfilepath = os.path.relpath(filepath, os.path.join(base_path, "src")).replace('\\', '/')
      if name.endswith(".js"):
...
# backends / extras, analogous to tinygrad runtime/viz
NONCORE_DIRS = {"minrl/inference"}
